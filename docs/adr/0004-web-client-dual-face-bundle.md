...
8. Web UI 必须继承 Host 的 verified/inferred/unattributed、observing/read-only/degraded 和 managed/external action 状态；Host 侧继续负责 fresh scan、身份复核、权限判断和 `portReleased` 结论。
9. 未知 DSH 版本、Client artifact 加载失败、Slot 不可用或 Host bridge 不可用时，Browser 只显示明确的只读/降级状态，不扩大进程操作权限。
