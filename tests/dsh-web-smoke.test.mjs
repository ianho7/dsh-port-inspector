...
test('real Stock DSH Web loads the Bundle, opens the panel, and rechecks an external action', {
  ...
  try {
    ...
    // Exercise the real DSH settings flow while the inspector remains open.
    ...
    await chineseConfirmation.getByRole('button', { name: '取消', exact: true }).click()
    await page.getByRole('button', { name: '设置', exact: true }).evaluate(element => element.click())
    const chineseSettings = page.getByRole('dialog', { name: '设置' })
