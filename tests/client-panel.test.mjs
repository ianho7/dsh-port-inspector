...
test('Runtime Inspector panel exposes semantic stable controls and all required state labels', () => {
  ...
  assert.match(source, /由 DSH 启动/)
  assert.match(source, /启动方未确认/)
  ...
  assert.match(source, /Agent ID/)
  assert.match(source, /Turn \/ Step/)
  assert.match(source, /Root Call ID/)
  assert.match(source, /Spawn 类型/)
  assert.match(source, /启动命令/)
...
test('development surface groups environments and keeps other listeners searchable and expandable', () => {
  ...
  assert.match(source, /`\$\{String\(currentProjectRows\.length\)\}项`/)
  assert.match(source, /`\$\{String\(developmentEnvironmentRows\.length\)\}项`/)
  assert.match(source, /`\$\{String\(pinnedRows\.length\)\}项`/)
  assert.match(source, /`\$\{String\(otherRows\.length\)\}项`/)
})
...
test('approved detail refinement collapses identical listeners and uses compact truthful fallbacks', () => {
  ...
  assert.match(source, /label: '相同监听'/)
  ...
  assert.match(source, /\?\? '—'/)
  assert.match(source, /compact \? '可停止' : '可停止 DSH 任务'/)
  assert.match(source, /compact \? '可结束' : '可结束进程'/)
})
