...
test('Runtime Inspector panel exposes semantic stable controls and all required state labels', () => {
  ...
  assert.match(source, /copyResultNotice\(result, row\.listenerId, row\.port, t\)/)
})
...
test('Compose associations show independent evidence and product/context logos', () => {
  ...
  assert.doesNotMatch(source, /snapshot\.composeStatus === 'unavailable'/)
  assert.doesNotMatch(source, /composeUnavailableBanner|sourceDegradedBanner|scanIncompleteBanner/)
...
test('installed panel keeps the approved hi-fi density and semantic palette under DSH host styles', () => {
  ...
  assert.match(stylesSource, /\.dsh-ri-notice\.is-error\s*\{[\s\S]*?background:\s*var\(--dsh-ri-danger-bg\)/su)
  assert.match(stylesSource, /\.dsh-ri-notice\.is-warning\s*\{[\s\S]*?background:\s*var\(--dsh-ri-warning-bg\)/su)
  assert.match(stylesSource, /\.dsh-ri-notice\.is-success\s*\{[\s\S]*?background:\s*var\(--dsh-ri-success-bg\)/su)
...
test('panel reuses only the same-context sidebar inventory while revalidating in the background', () => {
  assert.match(source, /interface EntryBadgeSnapshot\s*\{[\s\S]*?inventory\?: HostInventorySnapshot/su)
  assert.match(source, /publishEntryBadgeSnapshot[\s\S]*?inventory:\s*snapshot/su)
  assert.match(source, /sharedInventory\.contextKey === badgeContextKey[\s\S]*?sharedInventory\.inventory/su)
  assert.match(source, /state\.snapshotContextKey === badgeContextKey[\s\S]*?cachedSnapshot/su)
  assert.match(source, /if \(open\) refresh\(\)/)
})
