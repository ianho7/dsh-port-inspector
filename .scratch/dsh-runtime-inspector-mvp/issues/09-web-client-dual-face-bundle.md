...
- 2026-08-22: Added tests/dsh-web-smoke.test.mjs, an opt-in real Chromium smoke that stages the Bundle into a temporary Stock DSH Web Profile. With DSH_REPO=D:\project\deepseek-harness and DSH_WEB_E2E=1, DSH 0.1.1-rc.1 passed boot-graph/artifact loading, Sidebar entry, overlay panel, real inventory, external single-PID confirmation, Host identity recheck, process termination, portReleased=true, and a follow-up HTTP 200 proving the unaffected DSH Web listener remained alive. The 0.1.0-rc.8 run remains open because this checkout currently provides only the built 0.1.1-rc.1 Web Profile; the test is ready to rerun against a built rc.8 checkout.
