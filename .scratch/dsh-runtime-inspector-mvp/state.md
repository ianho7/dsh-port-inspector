...
attempts:
  ...
  - id: port-inspector-product-rename
    action: Rename the unpublished product, package, Bundle, Browser namespace, website, active documentation, design assets, fixtures, and release metadata from Runtime Inspector to Port Inspector.
    failure_class: none
    raw_evidence: "The active-tree old-name scan is empty outside historical handoffs/reflections and durable .scratch paths. npm test reports 164 tests: 161 passed, 3 opt-in native/Web gates skipped, 0 failed. Host emit, strict no-emit typecheck, Browser build, Astro static build, npm pack --dry-run, and git diff --check pass. The packed identity is dsh-port-inspector@0.1.1 with filename dsh-port-inspector-0.1.1.tgz, 70 files, and no old namespace in lib or website/dist."
    conclusion: "The first public release can use DSH Port Inspector and dsh-port-inspector consistently. API route, slots, RPC ids, storage key, DOM test attributes, website metadata, and test fixtures now share the new namespace; runtimeInspector domain symbols, dsh-ri CSS, and the read-only port_list contract remain intentionally unchanged."
    next_action_rule: "Rename the GitHub repository before publication, update the local origin and Cloudflare Pages binding, then publish npm before creating the GitHub Release from the same validated tarball."
evidence_gaps:
